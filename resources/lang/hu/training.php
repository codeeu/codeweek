<?php

return [

    'learning_bits' => 'Programozási hét – tudásmorzsák',
    'title' => 'Szeretne részt venni az európai programozási hét tevékenységeiben, de nem tudja, hol kezdje?',
    'text' => '<p>Itt ingyenes képzési anyagokat és forrásokat talál, amelyek segítenek az első lépésekben és a következő innovatív tanóra megtervezésében.</p>

                    <p>Nincs szükség előzetes programozási tapasztalatra, és mindegyik modul csak 15 percet vesz igénybe. A modulok a programozási és algoritmikus gondolkodási tevékenységekhez kapcsolódó kulcsfontosságú fogalmakat mutatják be. Ezenkívül a modulok gyakorlati tippeket és tanácsokat is adnak arra vonatkozóan, hogyan építheti be ezeket a fogalmakat a tanóra menetébe.</p>',
    'text_2' => '<p>Miután már egy vagy több tudásmorzsán is átrágta magát, reméljük, hogy elég magabiztosnak érzi magát ahhoz, hogy némi digitális kreativitással dobja fel az óráit, és megjelöli tevékenységét a <a href="/events">programozási hét térképén</a>!</p>

                    <p>Könnyen megszervezhet egy tematikus tanórát saját tantermében, vagy pedig egy nyílt napot vagy rendezvényt iskolájában. Mindössze találnia kell egy időpontot hozzá, majd regisztrálnia kell tevékenységét a <a href="/events">programozási hét térképén</a>. Minden tevékenységszervező részvételi oklevelet kap munkája elismeréseként.</p>

                    <p>Ha szeretne kapcsolatba lépni lelkes tanárok nemzetközi csoportjával, csatlakozzon az <a href="https://www.facebook.com/groups/774720866253044/?source_id=377506999042215">Európai programozási hét tanároknak szánt Facebook-csoportjához</a>! Ha az országon belül vagy határokon átívelő együttműködést szeretne kialakítani más iskolákkal, csatlakozzon a <a href="codeweek4all">Code Week 4 All kihíváshoz</a>.</p>',
    'lessons' => [
        '1' => [
            'title' => 'Programozás digitális technológia nélkül (offline)',
            'author' => 'Alessandro Bogliolo',
            'text' => '<p>A programozás a dolgok nyelve, amelynek segítségével olyan programokat írhatunk, amelyek új funkciókkal ruházzák fel a körülöttünk lévő milliárdnyi programozható tárgyat. A programozás az ötleteink megvalósításának leggyorsabb módja, és ez a leghatékonyabb módszer az algoritmikus gondolkodási képességek fejlesztésére. A számítástechnikai gondolkodásmód fejlesztéséhez azonban nincs feltétlenül szükség technológiára. A számítástechnikai gondolkodási készségeink azonban létfontosságúak a technológia működtetésére.</p>

                    <p>Ebben a videóban Alessandro Bogliolo, a számítógépes rendszereket kutató olasz tanár és az európai programozási hét nagykövete bemutatja azokat az offline programozási tevékenységeket, amelyek elektronikus eszköz nélkül gyakorolhatók. Az ilyen „offline” tevékenységek legfőbb célja, hogy csökkentse a programozáshoz való hozzáférés korlátait az iskolákban, és finanszírozástól és a berendezésektől függetlenül tegye lehetővé a programozást.</p>

                    <p>Az offline programozási tevékenységek a körülöttünk lévő fizikai világ számítástechnikai szempontjait fedik fel.</p>',
            'activities' => [
                '1' => '1. tevékenység: CodyRoby az általános iskolában',
                '2' => '2. tevékenység: CodyRoby a középiskola alsó tagozatában',
                '3' => '3. tevékenység: CodyRoby a középiskola felső tagozatában'
            ]
        ],
        '2' => [
            'title' => 'Algoritmikus gondolkodásmód és problémamegoldás',
            'author' => 'Miles Berry',
            'text' => '<p>A számítástechnikai gondolkodásmód (computational thinking, CT) a problémák és rendszerek olyan megismerési módját mutatja be, amelynek során számítógép segítségével megoldhatjuk vagy megérthetjük ezeket. Az számítástechnikai gondolkodásmód nemcsak a számítógépes programok fejlesztéséhez létfontosságú, hanem minden diszciplínában használják a problémamegoldás támogatására.</p>

                    <p>Úgy taníthat számítástechnikai gondolkodást a diákjainak, hogy megtanítja őket arra, hogy komplex problémákat kisebbekre osszák (felbontás), felismerjék a mintázatokat (mintázatfelismerés), azonosítsák a probléma megoldásában releváns részleteket (absztrakció), vagy olyan szabályokat vagy utasításokat határoz meg, amelyeket követni kell a kívánt eredmény elérése érdekében (algoritmikus tervezés). A számítástechnikai gondolkodás különböző tantárgyak keretén belül tanítható, például a matematikában (a másodfokú polinomok faktorizációs szabályainak felfedezése), az irodalomban (verselemzés felbontása a metrum, a ritmus és a struktúra elemzésére), a nyelvtanulásban (mintázatok keresése egy ige utolsó betűjére vonatkozóan, amelynek az igeidő módosulásával változik a helyesírása), és még sok más tárgyban.</p>

                    <p>Ebben a videóban Miles Berry, a University of Roehampton School of Education at Guildford (Egyesült Királyság) egyetemi docense bemutatja a számítástechnikai gondolkodást és azokat a lehetőségeket, amelyek révén a tanárok egyszerű játékok segítségével alkalmazhatják ezt a tanítás során.</p>',
            'activities' => [
                '1' => '1. tevékenység: A matematikai gondolkodás fejlesztése az általános iskolában',
                '2' => '2. tevékenység: Ismerkedés az algoritmusokkal a középiskola alsó tagozatában',
                '3' => '3. tevékenység: Algoritmusok a középiskola felső tagozatában'
            ]
        ],
        '3' => [
            'title' => 'Vizuális programozás – bevezetés a Scratch programozási nyelvbe',
            'author' => 'Margo Tinawi',
            'text' => '<p>A vizuális programozás lehetővé teszi az emberek számára, hogy illusztrációk vagy grafikus ábrázolás segítségével írják le a folyamatokat. Általában a szövegalapú programozással szembeállítva beszélünk vizuális programozásról. A vizuális programozási nyelvek (VPL) különösen alkalmasak arra, hogy segítségükkel bevezessék a gyermekeket (és a felnőtteket) az algoritmikus gondolkodásmódba. A vizuális programozási nyelvek segítségével kevesebb az olvasnivaló, és nincs végrehajtandó szintaxis.</p>

                    <p>A videóban Margo Tinawi, a Le Wagon webfejlesztést tanító tanára és a Techies Lab asbl (Belgium) társalapítója bemutatja a Scratch nyelvet, amely az egyik legnépszerűbb vizuális programozási nyelv a világon. A Scratch programozási nyelvet az MIT fejlesztette ki 2002-ben, azóta pedig hatalmas közösség jött létre körülötte. A közösségben milliónyi projektet találhat, amelyeket megismételhet a diákjaival, valamint számtalan oktatóanyag is elérhető különböző nyelveken.</p>

                    <p>A Scratch használatához nincs szükség programozási tapasztalatra, és mindenféle témakörben felhasználható! A Scratch digitális történetmesélési eszközként való használata esetén például a diákok történeteket hozhatnak létre, illusztrálhatnak egy matematikai problémát, vagy művészeti versenyben játszhatnak, amelynek során a kulturális örökségről tanulnak, miközben megismerkednek a programozással és a számítástechnikai gondolkodásmóddal, nem utolsósorban pedig szórakoznak.</p>

                    <p>A Scratch ingyenes eszköz, ráadásul rendkívül intuitív, és motiváló hatással van a diákokra. Tekintse meg Margo videóját, és ismerje meg az első lépéseket.</p>',
            'activities' => [
                '1' => '1. tevékenység: Scratch Basic az általános iskolában',
                '2' => '2. tevékenység: Scratch Basic a középiskola alsó tagozatában',
                '3' => '3. tevékenység: Scratch Basic a középiskola felső tagozatában'
            ]
        ],
        '4' => [
            'title' => 'Oktató jellegű játékok létrehozása a Scratch segítségével',
            'author' => 'Jesús Moreno León',
            'text' => '<p>A kritikai gondolkodás, a kitartás, a problémamegoldás, a számítástechnikai gondolkodásmód és a kreativitás csak néhány azon kulcsfontosságú készségek közül, amelyekkel diákjainak rendelkezniük kell ahhoz, hogy sikeresek legyenek a 21. században, a programozás pedig vidám és motiváló módon segít elérni ezt a célt.</p>

                    <p>Az áramlás-ellenőrzéssel kapcsolatos algoritmikus fogalmak, amelyek utasítások és hurkok sorát használják, változókat és listákat használó adatábrázolások vagy a folyamatok szinkronizálása bonyolult fogalmaknak tűnhetnek, de ebben a videóban meglátja, hogy sokkal könnyebb megtanulni őket, mint gondolná.</p>

                    <p>A videóban Jesús Moreno León, egy spanyolországi számítástechnikai tanár és kutató elmagyarázza, hogyan fejlesztheti ezeket és egyéb készségeket a diákjaiban szórakoztató módon. Hogy lehetséges ez? Kérdéseket és válaszokat hozhat létre az iskolákban világszerte használt legnépszerűbb programozási nyelvben, a Scratchben. A Scratch nemcsak a számítástechnikai gondolkodásmódot fejleszti, hanem lehetővé teszi a játékosítási elemek bevezetését is a tanórán, így a diákok motiváltak maradnak, miközben egyszerre tanulnak és szórakoztató tevékenységeket végeznek.</p>

                    <p>Tekintse meg a videót, és ismerje meg az első lépéseket:</p>',
            'activities' => [
                '1' => '1. tevékenység: Kérdések és válaszok a Scratch segítségével az általános iskolában',
                '2' => '2. tevékenység: Kérdések és válaszok a Scratch segítségével a középiskola alsó tagozatában',
                '3' => '3. tevékenység: Kérdések és válaszok a Scratch segítségével a középiskola felső tagozatában'
            ]
        ],
        '5' => [
            'title' => 'Robotika és bütykölés az iskolában',
            'author' => 'Tullia Urschitz',
            'text' => '<p>A programozás, a bütykölés, a robotika és a mikroelektronika az iskolai tantervbe tanítási és tanulási eszközként történő beemelése kulcsfontosságú a 21. századi oktatásban.</p>

                    <p>A bütykölés és a robotika iskolai használata számos előnnyel jár a diákok számára, mivel segít az olyan kulcskompetenciák fejlesztésében, mint a problémamegoldás, a csoportmunka és az együttműködés. Növeli a diákok kreativitását és önbizalmát, és segít fejleszteni a diákok kitartását és eltökéltségét kihívások esetén. A robotika is elősegíti az inkluzivitást, mivel könnyedén elérhető a különböző képességekkel és készségekkel rendelkező lányok és fiúk számára, és pozitívan befolyásolja az autizmus spektrumon lévő diákokat.</p>

                    <p>Tekintse meg az alábbi videót, amelyben Tullia Urschitz, az olasz Scientix nagykövete és Sant\'Ambrogio Di Valpolicella STEM-tanára gyakorlati példákon mutatja be, hogyan működhet az iskolai integráció a bütykölés és a robotika segítségével, amelynek során a passzív diákok lelkes alkotókká válnak.</p>',
            'activities' => [
                '1' => '1. tevékenység: Hogyan készítsünk mechanikus kezet kemény rostlemezből általános iskolában?',
                '2' => '2. tevékenység: Hogyan készítsünk mechanikus vagy robotkezet a középiskola alsó tagozatán?',
                '3' => '3. tevékenység: Hogyan készítsünk mechanikus vagy robotkezet a középiskola felső tagozatán?'
            ]
        ],
        '6' => [
            'title'=>'App Inventor és alkalmazásfejlesztés',
            'author'=>'Rosanna Kurrer',
            'text'=>'<p>Nézze meg az alábbi videót, amelyben Rosanna Kurrer (a CyberWayFinder alapítója) elmagyarázza, hogy mi az az App Inventor, felsorolja az alkalmazásfejlesztés előnyeit az oktatásban, és bemutat néhány gyakorlati példát arra, hogy a tanárok hogyan tudják beépíteni az App Inventort a tanórák menetébe, lelkes játékkészítőkké varázsolva a passzív diákokat.</p>
<p>Mindannyian használunk alkalmazásokat, mivel azok révén kényelmesen és gyorsan koordinálhatjuk tevékenységeinket. Egyre gyakrabban használják az alkalmazásokat az oktatásban is, mert lehetővé teszik a személyre szabott tanulást, valamint az információszerzést a felhasználók számára. Miért ne tehetné képessé diákjait is arra, hogy egy oktatást támogató alkalmazást hozzanak létre mások számára?</p>',
            'activities'=>[
                '1'=>'1. foglalkozás – Cím, általános iskolák számára',
                '2'=>'2. foglalkozás – Cím, középiskola alsó osztályai számára',
                '3'=>'3. foglalkozás – Cím, középiskola felső osztályai számára'
            ]
        ],
        '7' => [
            'title'=>'Bütykölés és alkotás',
            'author'=>'Diogo da Silva',
            'text'=>'<p>A foglalkozások és munkahelyek változnak, az oktatás pedig a nyomukban jár. A 21. századi szakmai pályafutásokra való felkészítés során olyan új készségek – mint például a bütykölés, az alkotás és a hekkelés képessége – válnak elengedhetetlenné, amelyek csökkentik a szakadékot az iskola és a valóság között. A diákok elköteleződésének és megfelelő fejlődésének elősegítése érdekében a tantermeket a problémamegoldásra összpontosító, együttműködő környezetté kell alakítani. E tevékenységek előmozdítják az eszmecserét, lehetővé téve ily módon az osztály számára, hogy minden hozzájárulást megbecsülő kommunikációs központtá váljon.</p> 
<p>Nézze meg az alábbi videót, amelyben a portugál TTMM-tanár és egyúttal az Escola Global tagja, Diogo da Silva, lépésről lépésre végigvezeti egy olyan tanóra felépítésén, amelyen diákjai bütykölés, alkotás és hekkelés útján jutnak el a végső megoldáshoz.</p>',
            'activities'=>[
                '1'=>'1. foglalkozás – Cím, általános iskolák számára',
                '2'=>'2. foglalkozás – Cím, középiskola alsó osztályai számára',
                '3'=>'3. foglalkozás – Cím, középiskola felső osztályai számára'
            ]
        ],
        '8' => [
            'title'=>'Programozás minden tantárgyban',
            'author'=>'M. Isabel Blanco, M. Concepción Fernández, Elisabetta Nanni, Debora Carmela Niutta, Stefania Altieri',
            'text'=>'<p>Amikor iskolai programozásra gondolunk, legelőször a számítástechnika, a technológia, a matematika vagy a természettudományok területe jut az eszünkbe. Mivel azonban a diákokat számtalan más dolog is érdekli és többféle tantárgyat tanulnak, miért ne fordítsuk ezt a magunk javára és vezessük be a programozást minden tantárgyban?</p> 
<p>Rengeteg előnye van annak, ha a programozást beépítjük a tanmenetbe: segít a diákoknak fejleszteni kritikai gondolkodásukat és problémamegoldó készségüket, támogatja, hogy aktív felhasználóvá váljanak és saját maguk irányítsák a tanulás folyamatát, ami elengedhetetlen az iskolában. De ami ezeknél is fontosabb, összeköthetik a kellemeset a hasznossal!</p>
<p>Idegen nyelvek, ének-zene, természettudományok, technológia, műszaki tudományok, matematika... és még folytathatnánk a sort. Tanárként minden tárgynál ott a lehetőség, hogy programozáson keresztül adjuk át az ismeretanyagot a jövő tudósai, zenészei, művészei, sőt bármely szakma képviselői számára! Minden tanár be tudja építeni a programozást és az algoritmikus gondolkodást saját tantervébe. Miért ne modellezhetnénk algoritmussal a napi rutint akár egy tartalomalapú nyelvórán? Természettudományos ismeretek átadásához készítsünk filcből játékvezérlőt! Egy oktatórobot a matematikához is meghozza a diákok kedvét. Megannyi lehetőség minden tanórán!</p> 
<p>Az alábbi kisfilmből megtudhatja, hogyan lehet könnyedén – akár minden előképzettség nélkül – beépíteni a programozást és az algoritmikus gondolkodást az ének-zene, a természettudományok vagy az idegen nyelvek oktatásába. </p>',
            'activities'=>[
                '1'=>'1. foglalkozás: Robotika és egyéni sikerélmény az általános iskolai TTMM-órákon',
                '2'=>'2. foglalkozás: A zene nem más mint programozás',
                '3'=>'3. foglalkozás: Bütykölés és programozás Makey Makey-vel',
                '4'=>'4. foglalkozás: Napi rutin algoritmusa tartalomalapú nyelvórán'
            ]
        ]
    ],
    'download_video_script' => 'Videó forgatókönyvének letöltése',
    'ready_to_share' => 'Készen áll arra, hogy megossza a tanultakat a diákjaival?',
    'choose_lessons' => 'Válasszon ki egyet az alábbi óravázlatok közül, és szervezzen tevékenységet a diákjaival.',
    'footer' => [
        'title' => 'Ne felejtse el tevékenységét feltenni a programozási hét térképére!',
        'text' => '<p>Könnyen szervezhet tematikus tanórát saját tantermében, vagy pedig egy nyílt napot vagy rendezvényt iskolájában. Mindössze találnia kell egy időpontot hozzá, majd regisztrálnia kell tevékenységét a <a href="/events">programozási hét térképén</a>. Minden tevékenységszervező részvételi oklevelet kap munkája elismeréseként.</p>

<p>Ha szeretne kapcsolatba lépni lelkes tanárok nemzetközi csoportjával, csatlakozzon az <a href="https://www.facebook.com/groups/774720866253044/?source_id=377506999042215">Európai programozási hét tanároknak szánt Facebook-csoportjához</a>! Ha az országon belül vagy határokon átívelő együttműködést szeretne kialakítani más iskolákkal, csatlakozzon a <a href="codeweek4all">Code Week 4 All kihíváshoz</a>.</p>'
    ]

];