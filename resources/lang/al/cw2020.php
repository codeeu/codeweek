<?php
return [
    "common" => [
        "resources" => "Burimet që do t’ju nevojiten",
        "soundtrack" => "Kolona zanore e Code Week",
        "moves"=> "Lëvizjet",
        "dance-example" => "Një shembull i kërcimit të Code Week",
        "organizer-guide" => "Udhëzuesi i organizatorit"
    ],
    "title" => [
        "EU Code Week",
        "Edicioni 2020"
    ],

    "intro" => "Këtë vit, festojmë kodimin nga 10 deri në 25 tetor 2020! Për shkak të situatës aktuale shëndetësore, do të sjellim më shumë aktivitete të Code Week në internet këtë vit. Do të prezantojmë aktivitete të reja argëtuese dhe përfshirëse në të cilat mund të bashkoheni në distancë në shtëpi ose në shkollë",
    "online-activities" => [
        "title" => "Aktivitetet e veçuara të hapura në internet",
        "subtitle" => [
            "Për ta bërë më të thjeshtë pjesëmarrjen në aktivitetet e Code Week në distancë, po prezantojmë një",
            "kalendar",
            "të aktiviteteve të veçuara në internet"
        ],
        "section1" => [
            "title" => "Çfarë janë aktivitetet në internet?",
            "content" => "Aktivitete në internet mund të jenë të gjitha aktivitetet që normalisht do t’i shtonit ose regjistronit në uebsajtin e Code Week – i vetmi ndryshim është se ato do të zhvillohen në internet. Ideja është që njerëzit ta kenë të lehtë pjesëmarrjen në sesionet dhe seminaret e kodimit krejtësisht në internet pa rrezikuar shëndetin e tyre"
        ],
        "section2" => [
            "title" => "Aktivitetet e veçuara të hapura",
            "content" => "Nëse dëshironi, mund t’i bëni aktivitetet tuaja në internet të disponueshme për këdo. Persona nga e gjithë bota do të jenë në gjendje të marrin pjesë në aktivitete të hapura në internet, qoftë në anglisht ose në gjuhën e tyre vendase. Ne do të zgjedhim aktivitetet më emocionuese në internet që janë të hapura për të gjithë dhe do t’i veçojmë ato në kalendarin e evenimenteve në uebsajtin tonë"
        ],
        "section3" => [
            "title" => "Çfarë do të thotë kjo për pjesëmarrësit",
            "content" => [
                "Çdo ditë të Code Week, do të jeni në gjendje të shfletoni",
                "kalendarin",
                "dhe të merrni pjesë në aktivitetet që ju interesojnë më shumë. Temat e atyre evenimenteve të veçuara nga seminarët rreth robotikës, kurset e mësimit në internet, uebinarët deri te tutorialët e kodimit e shumë më tepër. Pra, mund të zgjidhni"
            ]
        ],
    ],


    "dance" => [
        "title" => "Code Week Dance",
        "subtitle" => "Kush tha se programuesit nuk dinë të kërcejnë? Për të festuar edicionin e vitit 2020 të Code Week, po prezantojmë një aktivitet të ri - sfidën #EUCodeWeekDance",
        "section1" => [
            "title" => "Kush mund të bashkohet?",
            "content" => [
                "Të gjithë nga shkollat, mësuesit, bibliotekat deri te klubet e kodimit, bizneset dhe autoritetet publike janë të ftuar të festojnë EU Code Week 2020 duke organizuar një aktivitetet #EUCodeWeekDance dhe duke e shtuar në",
                "hartën e Code Week",
            ]
        ],
        "section2" => [
            "title" => "Si të merrni pjesë?",
            "content" => "Zgjidhni nga pesë lloje aktivitetesh ose sillni idetë tuaja. Pavarësisht se çfarë aktiviteti zgjidhni, mos harroni ta shtoni në hartën tonë"
        ],
        "activity1" => [
            "title" => "Programoni një shok ose prind, nuk nevojitet kompjuteri",
            "subtitle" => "Kodimi ju lejon t’i jepni komanda një pajisjeje elektronike. Por, teknikisht nuk ju nevojitet një kompjuter që të kodoni. Në vend të kësaj, merrni një partner – mund të jetë një shok klase, një shok, një prind ose edhe një mësues, dhe jepuni udhëzime se si të kryejnë #EUCodeWeekDance, që ata duhet ta ndjekin saktësisht",
            "resources" => [
                "Një udhëzues nga Code.org për të organizuar Dance Party jashtë linje",
                "Një mësim rreth mënyrës së programimit të robotit tuaj njeri",

            ]
        ],

        "activity2" => [
            "title" => "Programimi vizual",
            "subtitle" => "Kodoni Code Week Dance në Scratch. Përdorni personazhet e Code Week ose krijoni tuajat në Scratch dhe programojini ato të bëjnë #EUCodeWeekDance.",
            "resources" => [
                "Një shembull i një projekti Scratch të #EUCodeWeekDance",
                "Personazhet e Code Week",
                "Një udhëzues nga Code.org rreth mënyrës së animimit të një personazhi",
                "Një udhëzues nga Code.org për të organizuar Dance Party",
                "Ndani projektin tuaj me ne në këtë Scratch Studio."
            ]
        ], "activity3" => [
            "title" => "Programim me bazë teksti",
            "subtitle" => "Prodhoni temën e #EUCodeWeekDance me Python ose JavaScript, duke përdorur platforma krijimi muzike me bazë kodi si EarSketch ose Sonic Pi",
            "resources" => [
                "Një tutorial rreth mënyrës së prodhimit të muzikës me EarSketch",
                "Një tutorial rreth mënyrës së krijimit të muzikës me Sonic Pi",

            ]
        ],
        "activity4" => [
            "title" => "Robotika",
            "subtitle" => "Programojeni robotin tuaj të ndjekë udhëzimet tuaja dhe të kryejë Code Week Dance",
            "resources" => [
                "Një tutorial rreth programimit të robotëve",
            ]
        ], "activity5" => [
            "title" => "Sfida e kërcimit live",
            "subtitle" => "Regjistroni një video të vetes suaj, ekipit tuaj ose robotit tuaj duke kryer #EUCodeWeekDance, ndajeni në Instagram dhe provoni ta bëni videon virale dhe të fitoni disa shpërblime nga Code Week! Jeni i interesuar? Ndiqni këta hapa:",
            "resources" => [
                "Regjistrojeni videon duke përdorur Instagram Stories",
                "Ndiqni",
                "@CodeWeekEU në Instagram",
                "Përmendni @CodeWeekEU në Story me kërcimin dhe sigurohuni të përdorni hashtagun #EUCodeWeekDance",
            ]
        ],
        "outro" => [
            "Fituesit do të zgjidhen çdo ditë dhe do të prezantohen në kanalin tonë të Instagram përmes Stories, prandaj mos harroni njoftimet tuaja rregullisht, sot mund të jetë dita juaj me fat",
            "#EUCodeWeekDance bazohet në",
            "Ode to Code",
            "kompozuar nga Brendan Paolini dhe kërcimi i zhvilluar nga Bianca Maria Berardi në vitin 2015, nga ideja e Alessandro Bogliolo, Profesor i Sistemeve Kompjuterike pranë Universitetit të Urbinos"
        ],

    ],
    "treasure-hunt"=>[
        "title"=>"Code Week Treasure Hunt",
        "subtitle"=>[
            "Kjo është një lojë në Telegram që është mjaft e thjeshtë për fillestarët, por edhe sfiduese për t’i mbajtur të angazhuar edhe pjesëmarrësit me përvojë",
            "Code Week Treasure Hunt",
            "është loja më e mirë e luajtur në kompjuter me telefonin celular në dorë. Loja do t’ju kërkojë të zgjidhni sfidat e kodimit dhe do t’ju udhëzojë përmes historisë së kodimit, shkencës kompjuterike dhe teknologjisë në Evropë"
        ],
        "section" => [
            "title" => "Që të filloni të luani, ju nevojitet të",
            "content" => [
                "Shkarkoni aplikacionin Telegram. Është i disponueshëm për",
                "Desktop",
                "Lojën mund ta luani në kompjuter, në laptop ose në smartfon. Rekomandojmë që ta luani në kompjuter në mënyrë që të merrni udhëzimet dhe të zgjidhni sfidat e kodimit në aplikacionin Telegram në telefon",
                "Për të luajtur lojën",
                "hapni lojën",
                "dhe skanoni kodin QR që do t’ju çojë juve tek aplikacioni Telegram dhe do t’ju japë setin e parë të udhëzimeve",
                "Për të fituar, duhet të zgjidhni 10 sfida kodimi dhe të gjeni 10 vendndodhje në hartën e Evropës që lidhen me lindjen e kodimit dhe teknologjisë",
                "Pasi të përfundoni lojën, ndajeni rezultatin tuaj me miqtë duke përdorur #EUCodeWeek dhe sfidojini të luajnë dhe të mësojnë edhe ata rreth historisë së kodimit. Le të shohim kush arrin të ketë pikët më të larta",
                "Code Week Treasure Hunt është versioni virtual i vitit i lojës origjinale EU Code Week Treasure Hunt që u zhvillua fillimisht nga Alessandro Bogliolo, Profesor i Sistemeve Kompjuterike pranë Universitetit të Urbinos. Për të mësuar më shumë rreth lojës së tij origjinale, vizitoni",
                "blogun tonë"

            ]
        ],
    ],
    "kick-off"=>[
        "title"=>"Evenimenti prezantues: 8 tetor 2020",
        "content"=>[
            "Prezantimi i Code Week 2020 do të zhvillohet më 8 tetor, në orën 5 PM me orën e Berlinit/Brukselit/Parisit. Evenimenti do të transmetohet në Facebook Live, Instagram TV dhe YouTube Live",
            "Pjesëmarrësit tanë përfshijnë folës të ftuar",
            "Komisioneri i BE-së për Tregun e Brendshëm" ,
            "Komisioneri i BE-së për Inovacionin, Kërkimet, Kulturën, Arsimin dhe Rininë",
            "Krijuesi i Scratch dhe i Studimeve Mësimore pranë MIT Media Lab",
            "autor dhe ilustrues i Hello Ruby",
            "dhe",
            "Presidenti i organizatës jofitimprurëse franceze E-MMA që promovon diversitetin gjinor në teknologji",
            "Dhe nëse kjo nuk do të mjaftonte, ekipi i Code Week do të prezantojë gjithashtu veçoritë, burimet dhe sfidat e reja të uebsajtit",
            "Profesori i Sistemeve Kompjuterike pranë Universitetit të Urbinos dhe koordinator i",
            "Ambasadorëve të Code Week",
            "do t’ju tregojë më shumë rreth lojës virtuale Code Week Treasure Hunt",
            "Dhe xxx do t’ju japë informacione rreth sfidës së re #EUCodeWeekDance",
            "Ne do të lidhemi gjithashtu me shkolla dhe nxënës nga mbarë Evropa që do të ndajnë historitë e tyre të kodimit dhe do të shkëmbejnë ide me folësit e ftuar",
            "Ju do të keni gjithashtu mundësinë të ndani mendimet tuaja, idetë dhe të bëni pyetje. Gjithçka që ju nevojitet të bëni është të sintonizoheni në transmetimin tonë live në",
            "ose",
            "në 8 tetor 2020 në orën 17:00-18:30 CET dhe të komentoni ose të dërgoni tweet me hashtagun #EUCodeWeek",
        ]
    ],
    "get-involved"=>[
        "title"=>"Si të përfshiheni",
        "subtitle"=>"Mezi prisni të kodoni? Nëse doni të bashkoheni me Komunitetin e EU Code Week, por nuk dini ku të filloni, shikoni këto burime që do t’ju ndihmojnë të filloni, në kohë për festimin tonë vjetor në tetor",
        "content"=>[
            "Si të filloni me Code Week",
            "Si të shtoni një aktivitet të Code Week",
            "Mësimet",
            "Kursi masiv i hapur në internet Deep Dive",
            "Seria e Coding@Home"
        ]] ];