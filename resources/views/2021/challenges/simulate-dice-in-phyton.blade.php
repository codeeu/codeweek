                    ...
                    <div class="col-span-2">
                        <div class="px-6 py-8 bg-white rounded-lg lg:p-16">
                            ...
                            <div class="mb-6">
                                ...
                                {{-- Step 1 --}}
                                <strong class="block mb-2">{{ $items[0] ?? '' }}</strong>
                                <p class="mb-4 text-default md:text-xl text-slate-700">
                                    {{ $items[1] ?? '' }} {{ $items[2] ?? '' }} {{ $items[3] ?? '' }}
                                </p>

                                {{-- Step 2 --}}
                                <strong class="block mb-2">{{ $items[4] ?? '' }}</strong>
                                <ul class="mb-4 ml-6 list-disc text-slate-500">
                                    @foreach ([5, 6, 7] as $i)
                                        @if (!empty($items[$i]))
                                            <li>{{ $items[$i] }}</li>
                                        @endif
                        ...
                        {{ $items[15] ?? '' }}
                                ...
                                <p class="mb-4 text-slate-700">{{ $items[16] ?? '' }}</p>
                                <p class="mb-6 text-slate-700">{{ $items[17] ?? '' }}</p>

                                {{-- Step 3 --}}
                                <strong class="block mb-2">{{ $items[18] ?? '' }}</strong>
                                <ul class="mb-4 ml-6 list-disc text-slate-500">
                                ...
                                {{-- Step 4 --}}
                                <strong class="block mb-2">{{ $items[26] ?? '' }}</strong>
                                <ul class="mb-6 ml-6 list-disc text-slate-500">
                                    @foreach (range(27, 30) as $i)
                                        @if (!empty($items[$i]))
                                            <li>{{ $items[$i] }}</li>
                                        @endif
                                    @endforeach
                                ...
                                {{-- Quiz --}}
                                <strong class="block mb-2">@lang("challenges-content.$slug.quiz_title")</strong>
                                <ol class="mb-4 ml-6 list-decimal text-slate-500">
                                    @foreach (trans("challenges-content.$slug.quiz") as $quizItem)
                                        <li>{{ $quizItem }}</li>
                                    @endforeach
                                </ol>

                                {{-- Mini simulation --}}
                                <strong class="block mb-2">@lang("challenges-content.$slug.mini_simulation_title")</strong>
                                <ul class="mb-6 ml-6 list-disc text-slate-500">
                            ...
                            <div class="mb-6">
                                <p class="p-0 mb-2 text-2xl font-semibold">@lang("challenges-content.$slug.quiz_title")</p>
                                <ol class="list-decimal ml-4 [&_li]:my-2 leading-[22px] md:leading-[30px]">
                                    @foreach ($tarr("challenges-content.$slug.quiz") as $quiz)
