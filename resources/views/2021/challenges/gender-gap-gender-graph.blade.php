...
@php
    ...
    // Safe array translation helper (always returns an array)
...
@section('title', $title)
@section('description', trans("challenges-content.$slug.description"))
                        ...
                        <div class="flex flex-wrap gap-4 p-6 bg-white rounded-lg lg:flex-col 2xl:flex-row 2xl:gap-8">
                            ...
                            <div>
                                ...
                                <div class="flex gap-2 items-center px-4 py-1.5 bg-light-blue-100 rounded-full w-fit">
                                    ...
                                    <p class="p-0 font-semibold text-slate-500 text-default">
                                        @lang("challenges-content.$slug.experience")
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="p-6 bg-white rounded-lg">
                            ...
                            <ul class="ml-4 list-decimal">
                                @foreach ($tarr("challenges-content.$slug.materials") as $material)
                                    <a href="{{ $material }}"
                                        class="p-0 font-normal leading-7 cookweek-link hover-underline text-slate-500 text-default">{{ $material }}
                                    </a>
                                @endforeach
                            </ul>
                        </div>
                    ...
                    <div class="col-span-2">
                        <div class="px-6 py-8 bg-white rounded-lg lg:p-16">
                            ...
                            <div class="mb-6">
                                ...
                                {{-- strong => Manually (user level): (items[6], but ensure no embedded tags) --}}
                                @if (!empty($items[6]))
                                    <strong class="block mb-2 text-default md:text-xl">{{ strip_tags($items[6]) }}</strong>
                                @endif

                                {{-- p => items[7] --}}
                                @if (!empty($items[7]))
                                    <p class="mb-4 text-default md:text-xl text-slate-700">{{ $items[7] }}</p>
                                @endif
                                ...
                                @if (!empty($items[10]))
                                    <strong
                                        class="block mb-2 text-default md:text-xl">{{ strip_tags($items[10]) }}</strong>
                                @endif
                                ...
                                {{-- p => item[15] --}}
                                @if (!empty($items[15]))
                                    <p class="mb-6 text-default md:text-xl text-slate-700">{{ $items[15] }}</p>
                                @endif
                                ...
                                {{-- strong + p from item[16] (split at colon) --}}
                                @if (!empty($items[16]))
                                    @php
                                        [$head, $rest] = $splitAtColon($items[16]);
                                    @endphp
                                    <strong class="block mb-2 text-default md:text-xl">{{ $head }}</strong>
                                    @if ($rest)
                                        <p class="mb-4 text-default md:text-xl">{{ $rest }}</p>
                                    @endif
                                @endif

                                {{-- strong => Simple challenge: (17) + p (18) --}}
                                @if (!empty($items[17]))
                                    <strong
                                        class="block mb-2 text-default md:text-xl">{{ strip_tags($items[17]) }}</strong>
                                @endif
                                @if (!empty($items[18]))
                                    <p class="mb-6 text-default md:text-xl">{{ $items[18] }}</p>
                                @endif

                                {{-- strong => Advanced challenge: (19) + p combine (20..22) --}}
                                @if (!empty($items[19]))
                                    <strong
                                        class="block mb-2 text-default md:text-xl">{{ strip_tags($items[19]) }}</strong>
                                @endif
                                @php
                                ...
                                @endphp
                                @if ($adv)
                                    <p class="mb-6 text-default md:text-xl">{{ $adv }}</p>
                                @endif

                                {{-- strong + p from Tip (23) --}}
                                @if (!empty($items[23]))
                                    @php
                                        [$head, $rest] = $splitAtColon($items[23]);
                                    @endphp
                                    <strong class="block mb-2 text-default md:text-xl">{{ $head }}</strong>
                                    @if ($rest)
                                        <p class="mb-4 text-default md:text-xl">{{ $rest }}</p>
                                ...
                                {{-- p => item[24] --}}
                                @if (!empty($items[24]))
                                    <p class="mb-4 text-default md:text-xl">{{ $items[24] }}</p>
                                @endif

                                {{-- ul with nested list: 25, 26, 27 + children 28..29 --}}
                                <ul class="mb-6 ml-6 list-disc text-slate-500 text-default md:text-xl">
                                    @if (!empty($items[25]))
                                        <li>{{ $items[25] }}</li>
                                    @endif
                                ...
                                @foreach ([30, 31] as $i)
                                    @if (!empty($items[$i]))
                                        <p class="mb-4 text-default md:text-xl">{{ $items[$i] }}</p>
                                    @endif
                                @endforeach

                                {{-- strong => 32, p => 33 (solution link as plain text) --}}
                                @if (!empty($items[32]))
                                    <strong class="block mb-2 text-default md:text-xl">{{ $items[32] }}</strong>
                                @endif
                                @if (!empty($items[33]))
                                    <p class="mb-6 text-default md:text-xl">{{ $items[33] }}</p>
                                @endif
                                <img width="100%" src="/images/challenges/gender-gap-gender-graph/4.png">
                                {{-- strong => Discussion: (34), then ordered list 35..39 --}}
                                @if (!empty($items[34]))
                                    <strong
                                        class="block mb-2 text-default md:text-xl">{{ strip_tags($items[34]) }}</strong>
                                @endif
                                ...
                                {{-- strong => More questions: (40), ul => 41..43 --}}
                                @if (!empty($items[40]))
                                    <strong
                                        class="block mb-2 text-default md:text-xl">{{ strip_tags($items[40]) }}</strong>
                                @endif
                                <ul class="mb-6 ml-6 list-disc text-slate-500 text-default md:text-xl">
                                ...
                                {{-- final takeaway => 44 --}}
                                @if (!empty($items[44]))
                                    <p class="text-default md:text-xl text-slate-700">{{ $items[44] }}</p>
                                @endif
                            </div>

                            @include('2021.challenges._share')
                            <div class="mb-6">
                                <p class="p-0 mb-2 text-2xl font-semibold">@lang("challenges-content.$slug.real-life-applications_title")</p>
