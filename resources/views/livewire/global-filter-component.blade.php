<div class="codeweek-container-lg mx-auto">
    ...
    <div>
        <nav class="flex flex-wrap gap-2.5 justify-start items-start text-base text-black rounded-[32px] max-md:max-w-full">
            ...
            <a href="#" wire:click.prevent="selectFilter('{{ $filter }}')" class="gap-2.5 self-stretch text-[16px] leading-5 px-6 py-2.5 {{ $selectedFilter === $filter ? 'bg-[#1C4DA1] text-white' : 'bg-[#CCF0F9] hover:bg-[#98E1F5] text-black' }} duration-300 rounded-3xl max-md:px-5">
                {{ $filter }}
            </a>
            @endforeach
        </nav>
